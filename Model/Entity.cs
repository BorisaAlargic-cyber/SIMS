...
namespace Model
{
    public abstract class Entity : ViewModelBase, IDataErrorInfo
    {
        ...
        public bool IsLenghtValid(string propertyValue, int maxLenght, out string error)
        {
            if (propertyValue.Length > maxLenght)
            {
                error = string.Format("Maksimalna duzina unetog teksta ne sme biti veca od {0} karaktera", maxLenght);
                return false;
            }
